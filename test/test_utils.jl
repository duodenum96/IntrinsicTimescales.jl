...
@testset "find_knee_frequency tests" begin
    ...
    @testset "Ideal Lorentzian" begin
        ...
        @test isapprox(detected_knee, f_knee, rtol=0.001)
    end
    ...
    @testset "Noisy Lorentzian" begin
        ...
        @test isapprox(detected_knee, f_knee, rtol=0.05)
    end
    ...
    @testset "Variable exponent" begin
        ...
        amp = 100.0
        exponent = 1.5
        psd = @. amp / (1 + ((freqs / f_knee)^exponent))
        psd2 = @. amp / (1 + ((freqs / f_knee)^2))

        estimated_amp, estimated_knee, estimated_exponent = find_knee_frequency(psd, freqs,
                                                                                allow_variable_exponent=true,
                                                                                constrained=true)
        @test isapprox(estimated_knee, f_knee, rtol=0.01)
        @test isapprox(estimated_exponent, exponent, rtol=0.01)
    end
    ...
    @testset "Different frequency ranges" begin
        ...
        knee2 = find_knee_frequency(psd, freqs, min_freq=1.0 / 1000.0,
                                    max_freq=20.0 / 1000.0)[2]

        @test isapprox(knee1, knee2, rtol=0.01)
        @test isapprox(knee1, f_knee, rtol=0.01)
    end
    ...
    @testset "OU process" begin
        ...
        average_psd = mean(psd, dims=1)
        detected_knee = find_knee_frequency(average_psd[:], freqs, min_freq=freqs[1],
                                            max_freq=freqs[end])[2]
        # Theoretically, tau = 1 / 2pi * f_knee
        @test isapprox(tau, (1 ./ (2π .* detected_knee)), rtol=0.1)
        ...
        detected_knee = find_knee_frequency(average_psd[:], freqs, min_freq=freqs[1],
                                            max_freq=freqs[end])[2]
        # Theoretically, tau = 1 / 2pi * f_knee
        @test isapprox(true_tau, (1 / (2π * detected_knee)), rtol=0.1)
    end
end
...
@testset "N-dimensional array handling" begin
    ...
    @testset "find_knee_frequency with different dimensions" begin
        ...
        detected_knees_2d_parallel = find_knee_frequency(psd_2d, freqs, dims=2,
                                                         parallel=true)[:, 2]
        ...
        detected_knees_3d_parallel = find_knee_frequency(psd_3d, freqs, dims=1,
                                                         parallel=true)[2, :, :]
    ...
    @testset "fooof_fit with different dimensions" begin
        ...
        result_1d_single_peak = fooof_fit(perfect_psd, freqs, oscillation_peak=true,
                                          max_peaks=1)
        ...
        results_2d_parallel = fooof_fit(psd_2d, freqs, dims=2, oscillation_peak=true,
                                        max_peaks=2, parallel=true)
        ...
        results_3d_parallel = fooof_fit(psd_3d, freqs, dims=1, oscillation_peak=true,
                                        max_peaks=2, parallel=true)
