...
tr.@model function fit_psd_with_osc(data, freq, fs)
    ...
    freq_osc ~ tr.truncated(tr.Normal(20.0, 5.0), 0.0, Inf)
    coeff ~ tr.truncated(tr.Normal(0.5, 0.3), 0.0, 1.0)
    σ ~ tr.Exponential(1)
    
    # Generate predicted data
    theta = [tau, freq_osc, coeff]
...
model = fit_psd_with_osc(data_psd, freq, fs)
advi = tr.ADVI(10, 100, AutoZygote())
